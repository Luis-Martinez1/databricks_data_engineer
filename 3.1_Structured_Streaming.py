...
# This code is inserting data into the "books" table. The values being inserted are for three books, along with their respective attributes such as ID, title, author, category, and price. Each book has its own set of values, and they are being inserted into the table using the INSERT INTO statement.
...
# The SQL code is inserting data into the "books" table. The code specifies the values to be inserted for each row.

#The first row being inserted has the values "B16" for the book ID, "Hands-On Deep Learning Algorithms with Python" for the book title, "Sudharsan Ravichandiran" for the author, "Computer Science" for the genre, and 25 for the price.

#The second row being inserted has the values "B17" for the book ID, "Neural Network Methods in Natural Language Processing" for the book title, "Yoav Goldberg" for the author, "Computer Science" for the genre, and 30 for the price.

#The third row being inserted has the values "B18" for the book ID, "Understanding digital signal processing" for the book title, "Richard Lyons" for the author, "Computer Science" for the genre, and 35 for the price.
...
# This code snippet uses Apache Spark's structured streaming API to write the data from a temporary view called "author_counts_tmp_vw" to a table called "author_counts".
...
# The SQL code is selecting all columns from the "author_counts" table and ordering the result by the second column in descending order (from highest to lowest).
%sql
SELECT *
FROM author_counts
order by 2 desc
...
# The code is selecting all the data from a view called "books_streaming_tmp_vw". It does not drop the view, but it is commented out

%sql
--drop view books_streaming_tmp_vw
select * from  books_streaming_tmp_vw
